...
$(window).on('load', function () {
  let stateOption = "<option value='' selected disabled>Select State</option>";
  for (i = 0; i < state_arr.length; i++) {
    stateOption +=
      "<option value='" + state_arr[i] + "' > " + state_arr[i] + '</option>';
  }
  $('#state,#permanentstate').html(stateOption);
});
$('#state,#permanentstate').on('change', function () {
  let cityArr = s_a[state_arr.indexOf($(this).val()) + 1];
  cityArr = cityArr.split(' | ');
  let cityOptions = "<option value='' selected disabled>Select City</option>";
  for (i = 0; i < cityArr.length; i++) {
    cityOptions +=
      "<option value='" + cityArr[i] + "' > " + cityArr[i] + '</option>';
  }
  $('#' + $(this).attr('data-city')).html(cityOptions);
});
...
$('#customCheck1').click(function () {
  if ($('#customCheck1').is(':checked')) {
    $('#perminputAddress').val($('#inputAddress').val());
    $('#perminputAddress2').val($('#inputAddress2').val());
    $('#perminputAddress').val($('#inputAddress').val());
    let option =
      "<option value='" +
      $('#state').val() +
    ...
    option =
      ...
      $('#city').val() +
      '</option>';
    $('#permanentcity').html(option);
    $('#perminputZip').val($('#inputZip').val());
    document
      .getElementById('perminputAddress')
      .setAttribute('disabled', 'true');
    document
      .getElementById('perminputAddress2')
      .setAttribute('disabled', 'true');
    document.getElementById('permanentcity').setAttribute('disabled', 'true');
    document.getElementById('permanentstate').setAttribute('disabled', 'true');
    document.getElementById('perminputZip').setAttribute('disabled', 'true');
  } else {
    document.getElementById('perminputAddress').removeAttribute('disabled');
    document.getElementById('perminputAddress2').removeAttribute('disabled');
    document.getElementById('permanentcity').removeAttribute('disabled');
    document.getElementById('permanentstate').removeAttribute('disabled');
    document.getElementById('perminputZip').removeAttribute('disabled');
  }
});
...
$(document).ready(function () {
  $('select[name="route_id"]').change(function (event) {
    routeID = this.value;
    $.ajax({
      type: 'GET',
      ...
      success: function (response) {
        ...
        response.forEach(function (shift) {
          let shift_name =
            shift.route_code_1 === null ? 'Afternoon' : 'Morning';
          $input.append(`<option value='${shift.id}'>${shift_name}</option>`);
        });
        if (response.length === 2) {
          $input.append(`<option value='both'>Both</option>`);
        }
      },
    });
  });
